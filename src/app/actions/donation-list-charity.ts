...
export async function reviewDonationListItem(
  ...
  try {
    ...
    if (typeof itemId !== "string" || !itemId) {
      return { error: actionError("MISSING_ITEM_ID", "Missing item.") };
    }
    ...
    if (!item || item.donationList.status !== DonationListStatus.SUBMITTED) {
      return {
        error: actionError(
          "ITEM_NOT_REVIEWABLE",
          "Item not found or list is not submitted.",
        ),
      };
    }
    ...
    return { error: toActionFailure(e, "REVIEW_ITEM_FAILED") };
  }
}
...
export type CharityResponseFormState = ActionFormState;
...
export async function submitCharityResponseToContributor(
  ...
  try {
    ...
    if (typeof listId !== "string" || !listId) {
      return { error: actionError("MISSING_LIST_ID", "Missing list.") };
    }
    if (typeof messageRaw !== "string") {
      return { error: actionError("MISSING_MESSAGE", "Enter a message.") };
    }
    const parsed = responseMessageSchema.safeParse(messageRaw);
    if (!parsed.success) {
      const msg = parsed.error.flatten().formErrors[0] ?? "Invalid message.";
      return { error: actionError("INVALID_MESSAGE", msg) };
    }
    ...
    if (!list) {
      return { error: actionError("LIST_NOT_FOUND", "List not found.") };
    }
    if (list.items.length === 0) {
      return {
        error: actionError(
          "EMPTY_LIST",
          "This list has no items to review.",
        ),
      };
    }
    ...
    if (!allReviewed) {
      return {
        error: actionError(
          ...
          "All items must be reviewed first.",
        ),
      };
    }
    ...
    if (isRedirectError(e)) throw e;
    return { error: toActionFailure(e, "SEND_RESPONSE_FAILED") };
  }
}
